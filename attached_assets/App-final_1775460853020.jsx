...
// ─── LIVE PRICE HOOK ────────────────────────────────────────────────
function useLivePrices(tickers, refreshInterval = 60000) {
  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      setLoading(true);
      try {
        const data = await api("/quotes", { method: "POST", body: JSON.stringify({ tickers }) });
...
// ─── LIVE PRICE DISPLAY ─────────────────────────────────────────────
function LivePrice({ quote, fallbackPrice }) {
  if (!quote) {
    return <span style={{ fontFamily: M, fontSize: 13, fontWeight: 600 }}>${fallbackPrice}</span>;
  }
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
      <span style={{ fontFamily: M, fontSize: 13, fontWeight: 700 }}>${quote.price.toFixed(2)}</span>
      <span style={{ fontSize: 11, fontWeight: 600, color, fontFamily: M }}>
...
function PriceTimestamp({ timestamp }) {
  if (!timestamp) return null;
  const date = new Date(timestamp);
  const timeStr = date.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  return <span style={{ fontSize: 10, color: C.mut, fontFamily: F }}>Live · {timeStr}</span>;
}
...
export default function App() {
  ...
  const saveAndRun = async (cs) => {
    try {
      await api("/criteria", { method: "POST", body: JSON.stringify(cs) });
      await loadCriteria();
      setEditingSet(null);
      showToast(`Saved "${cs.name}" — running screen...`);
      await runScreen(cs);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: F }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px", minHeight: "calc(100vh - 120px)" }}>
        ...
        {!loading && page === "criteria-library" && <CriteriaLibrary sets={criteriaSets} onNew={() => { setEditingSet(null); setPage("criteria-builder"); }} onEdit={cs => { setEditingSet(cs); setPage("criteria-builder"); }} onDelete={deleteCriteria} onRun={runScreen} onSaveAndRun={saveAndRun} />}
        {page === "criteria-builder" && <CriteriaBuilder existing={editingSet} onSave={saveCriteria} onSaveAndRun={saveAndRun} onCancel={() => setPage("criteria-library")} />}
...
function StocksPage({ onViewStock, onToast, watchlist, onToggleWL }) {
  ...
  const { prices: livePrices } = useLivePrices(stocks.map(s => s.ticker));
            ...
            <tbody>
              {filtered.map(s => (
                <tr key={s.ticker} style={{ borderBottom: `1px solid ${C.bdr}`, transition: "background 0.1s" }}
                  ...
                  <td style={{ padding: "10px 14px" }}><LivePrice quote={livePrices[s.ticker]} fallbackPrice={s.price} /></td>
...
// CRITERIA LIBRARY
// ═══════════════════════════════════════════════════════════════════
function CriteriaLibrary({ sets, onNew, onEdit, onDelete, onRun, onSaveAndRun }) {
  return (
    <div>
      ...
      <div style={{ marginTop: 8 }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, color: C.txt, marginBottom: 6 }}>Quick-Start Templates</h2>
        <p style={{ fontSize: 13, color: C.mut, margin: "0 0 14px 0" }}>One click to save and instantly screen all stocks in your database.</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12 }}>
          {TEMPLATES.map((t, i) => (
            <Card key={i} style={{ padding: 16 }}>
              ...
              <Btn sz="sm" v="acc" icon="play" onClick={() => {
                onSaveAndRun({ id: `cs-${Date.now()}`, name: t.name, criteria: JSON.parse(JSON.stringify(t.criteria)) });
              }}>Run Screen</Btn>
            </Card>
          ))}
        </div>
...
// CRITERIA BUILDER
// ═══════════════════════════════════════════════════════════════════
function CriteriaBuilder({ existing, onSave, onSaveAndRun, onCancel }) {
  ...
  const validate = () => {
    ...
    if (!criteria.length) { setErr("Add at least one criterion."); return null; }
    return { id: existing?.id || `cs-${Date.now()}`, name: name.trim(), criteria };
  };

  const save = () => { const cs = validate(); if (cs) onSave(cs); };
  const saveAndRun = async () => {
    const cs = validate();
    if (cs) { setBusy(true); await onSaveAndRun(cs); setBusy(false); }
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 20, justifyContent: "flex-end" }}>
        ...
        <Btn v="out" onClick={save} icon="check">Save Only</Btn>
        <Btn v="acc" onClick={saveAndRun} disabled={busy} icon="play">{busy ? "Running..." : "Save & Run Screen"}</Btn>
      </div>
    </div>
  );
}
...
function Results({ results, critSet, watchlist, onToggleWL, onSelect, onBack }) {
  ...
  const { prices: livePrices } = useLivePrices((results || []).map(r => r.ticker));
            ...
            <tbody>
              {filtered.map((s, i) => (
                <tr key={s.ticker} onClick={() => onSelect(s)} style={{ cursor: "pointer", borderBottom: `1px solid ${C.bdr}`, transition: "background 0.1s" }}
                  ...
                  <td style={{ padding: "11px 14px" }}><LivePrice quote={livePrices[s.ticker]} fallbackPrice={s.price} /></td>
...
function Analysis({ stock, critSet, isWL, onToggleWL, onBack }) {
  ...
  const { prices: livePrices } = useLivePrices([stock.ticker], 30000);
  const liveQuote = livePrices[stock.ticker];
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 4, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <LivePrice quote={liveQuote} fallbackPrice={stock.price} />
              {liveQuote && <PriceTimestamp timestamp={liveQuote.timestamp} />}
      ...
      {/* Live Quote Bar */}
      {liveQuote && (
        <Card style={{ marginBottom: 16, padding: "12px 20px", background: liveQuote.change >= 0 ? "#F0FDF4" : "#FEF2F2", border: `1px solid ${liveQuote.change >= 0 ? C.okBdr : C.errBdr}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
              <div>
                <div style={{ fontSize: 10, color: C.mut, textTransform: "uppercase", letterSpacing: "0.5px" }}>Open</div>
                <div style={{ fontSize: 14, fontWeight: 700, fontFamily: M }}>${liveQuote.open.toFixed(2)}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: C.mut, textTransform: "uppercase", letterSpacing: "0.5px" }}>Day High</div>
                <div style={{ fontSize: 14, fontWeight: 700, fontFamily: M, color: "#059669" }}>${liveQuote.high.toFixed(2)}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: C.mut, textTransform: "uppercase", letterSpacing: "0.5px" }}>Day Low</div>
                <div style={{ fontSize: 14, fontWeight: 700, fontFamily: M, color: "#DC2626" }}>${liveQuote.low.toFixed(2)}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: C.mut, textTransform: "uppercase", letterSpacing: "0.5px" }}>Prev Close</div>
                <div style={{ fontSize: 14, fontWeight: 700, fontFamily: M }}>${liveQuote.prevClose.toFixed(2)}</div>
              </div>
            </div>
            <PriceTimestamp timestamp={liveQuote.timestamp} />
          </div>
        </Card>
      )}
    ...
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {Object.keys(livePrices).length > 0 && <PriceTimestamp timestamp={Object.values(livePrices)[0]?.timestamp} />}
        </div>
      </div>
      ...
      {stocks.length === 0 ? (
        ...
        <Card style={{ padding: 0, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: F }}>
            <thead><tr style={{ background: C.bg }}>
              {["Ticker", "Company", "Sector", "Price", "Change", "P/E", "Div Yield", ""].map((h, i) => (
            ...
            <tbody>
              {stocks.map(s => {
                ...
                <tr key={s.ticker} onClick={() => onSelect(s)} style={{ cursor: "pointer", borderBottom: `1px solid ${C.bdr}`, transition: "background 0.1s" }}
                  ...
                  <td style={{ padding: "11px 14px", fontSize: 13, color: C.txt }}>{s.name}</td>
                  <td style={{ padding: "11px 14px" }}><Badge>{s.sector}</Badge></td>
                  <td style={{ padding: "11px 14px", fontWeight: 700, fontFamily: M, fontSize: 13 }}>
                    ${q ? q.price.toFixed(2) : s.price}
                  </td>
                  <td style={{ padding: "11px 14px", fontFamily: M, fontSize: 12, fontWeight: 600, color: q ? (q.change >= 0 ? "#059669" : "#DC2626") : C.mut }}>
                    {q ? `${q.change >= 0 ? "▲" : "▼"} ${Math.abs(q.changePercent).toFixed(2)}%` : "—"}
                  </td>
